...
class TrafficLightDQN:
    ...
    def __init__(self, memo, f_prefix):
        ...
        self.agent = self.DIC_AGENTS[self.para_set.MODEL_NAME](num_phases=8,
                                                               num_actions=8,
    ...
    def train(self, sumo_cmd_str, if_pretrain, use_average):
        ...
        while current_time < total_run_cnt:
            ...
            state_CTT = "["
            for i in range(11):
                state_CTT += "{0} ".format(state.cumulative_travel_time[0][i])
            state_CTT += "{0}]".format(state.cumulative_travel_time[0][11])


            # output to std out and file
            memory_str = 'T;%d;\tA;%d;\tCP;%d;\tNP;%d;\tR;%f;\tQ;%f;%f;%f;%f;%f;%f;%f;%f' \
                         % (current_time, action,
                            state.cur_phase[0][0],
                            state.next_phase[0][0],
                            reward, q_values[0][0], q_values[0][1], q_values[0][2], q_values[0][3],
                            q_values[0][4], q_values[0][5], q_values[0][6], q_values[0][7])
            print(memory_str)
            ...
            #a = map_computor.get_all_vehicle_info()
